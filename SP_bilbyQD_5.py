...
outdir = 'outdir_simulated_alpha1fixed'
...
def model(x,f,mu1,sigma1,alpha1,mu2,sigma2,alpha2,A):
#def model(x,mu1,sigma1,alpha1):
    ...
    for unit in x:
       #result = integrate.quad(lambda xdash: (1/(np.sqrt(2*(np.pi**2))))*np.exp(-0.5*(xdash**2))*gauss1(unit-xdash,f,mu1,sigma1,A1,alpha1),-np.inf,np.inf)[0]
        ...
        #2forslurm
        C1 = alpha1/((2**(1+(1/alpha1)))*sigma1*)
        result = integrate.quad(lambda xdash: (1/(np.sqrt(2*np.pi)))*np.exp(-0.5*(xdash**2))*((f*((1/np.sqrt(2*np.pi*sigma1**2))*np.exp(-0.5*np.abs(((unit-xdash)-mu1)/sigma1)**alpha1)))+((A*(1-f))*(1/np.sqrt(2*np.pi*sigma2**2))*np.exp(-0.5*np.abs(((unit-xdash)-mu2)/sigma2)**alpha2))),x.min(),x.max())[0]
        ...
        #result = integrate.simps((1/(np.sqrt(2*(np.pi**2))))*np.exp(-0.5*(xdash**2))*gauss1(unit-xdash,f,mu1,sigma1,A1,alpha1),xdash)
