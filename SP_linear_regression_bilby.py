...
def model(x,f,mu1,sigma1,A1,alpha1,mu2,sigma2,A2,alpha2):
    ...
    xmax = x.max()
    for unit in x:
        result = integrate.quad(lambda xdash: (1/(np.sqrt(2*(np.pi**2))))*np.exp(-0.5*(xdash**2))*bimodal(unit-xdash,f,mu1,sigma1,A1,alpha1,mu2,sigma2,A2,alpha2),xmin,xmax)[0]
...
priors['sigma1'] = bilby.core.prior.Uniform(0.5, 5, 'sigma1')
priors['A1'] = bilby.core.prior.Uniform(1000, 5000, 'A1')
...
priors['A2'] = bilby.core.prior.Uniform(1000, 5000, 'A2')
priors['alpha2'] = bilby.core.prior.Uniform(1, 100, 'alpha2')
